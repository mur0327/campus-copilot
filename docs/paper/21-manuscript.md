...
**작성일 2026-07-17, 최종 갱신 2026-07-23.**
...
`[[ ]]` 표기는 미기입 자리표시자다.
본문 인용은 REFERENCE의 등장 순 번호 표기다(2026-07-23 임시 키 일괄 치환·검증 완료, 키 대응은 git 이력 참조). 문헌을 추가·삭제하면 등장 순 번호를 재부여해야 한다. 기입된 쪽수와 저자진은 최종 감사에서 전수 재확인한다.
...
대학의 학사 정보는 학사 안내 페이지, 공지사항, 학과 및 부속 기관 사이트에 흩어져 있으며, 같은 주제를 다루는 문서라도 적용 시점과 대상 집단이 서로 다르다. 본부 안내를 옮겨 실은 학과 페이지, 회차마다 갱신되는 모집 공지, 신입생이나 졸업예정자와 같이 특정 집단만을 위한 안내가 하나의 코퍼스(corpus) 안에 공존한다. 검색 증강 생성(Retrieval-Augmented Generation, RAG)[1]은 이러한 공식 문서를 근거로 질문에 답하는 접근으로 대학 학사 안내에도 적용되고 있으나[2,3], 관련 문서의 검색에 성공하였다고 해서 사용자에게 그대로 내보낼 수 있는 답변이 보장되는 것은 아니다.

이는 검색 회수의 성공과 최종 응답의 적합성이 서로 다른 문제이기 때문이다. 문서 단위의 관련성 라벨이 하위 생성 품질을 보장하지 않는다는 지적이 제기되어 왔으며[4], 정답 페이지가 상위에 회수되더라도 실제 주입된 청크(chunk)가 질문의 핵심 주장을 지지하지 않을 수 있다. 또한 생성된 문장이 근거에 지지되더라도 표시 출처가 부정확할 수 있고[5], 내용이 사실이더라도 지난 회차의 안내이거나 다른 집단을 위한 문서라면 현재 답변의 근거로 사용하기 어렵다[6]. 나아가 근거의 충분 여부와는 별개로, 완전 답변, 제한 답변과 거절 중에서 적절한 행동을 선택하는지도 독립된 평가 대상이다[7,8]. 이러한 층위들을 하나의 점수로 합치면 실패의 위치를 특정할 수 없다.

이에 본 연구는 단일 대학의 공식 웹 문서 코퍼스와 운영 RAG 경로를 동결하고, 목적 표집한 고정 50문항에서 이러한 층위들을 분리하여 관찰하는 오프라인 사례 연구를 수행한다. 질문, 페이지 및 청크 세 수준의 평가 정답표(qrel)에 페이지 지지, 청크 근거, 시간 배포 가능성, 대상자 적합성과 질문별 기대 행동을 분리하여 기록하고, LLM 전량 초벌 판정과 초기 라벨 비공개 사람 감사, 전체 문항 검토와 불일치 조정을 거쳐 최종 평가 정답표를 확정한다. 같은 최종 평가 정답표로 BM25 단독, Semantic 단독과 Hybrid의 세 검색 구성을 재채점하고, 운영 Hybrid 경로의 최종 응답은 통제 평가 화면에서 재현한 답변 텍스트와 표시 출처로 판정하며, 같은 질문을 세 번 실행하여 구조화 행동의 반복성을 진단한다. 모든 수치와 실패 빈도는 단일 기관에서 코퍼스와 평가 시점을 고정한 이 사례에 한정된다.

본 연구의 연구문제는 다음과 같다. RQ1은 공통 후처리를 공유하는 세 검색 구성이 페이지 및 근거 회수와 순위 품질에서 어떤 차이를 보이는지 검토한다. RQ2는 최종 응답에서 내용 정확성, 근거 축과 배포 적합성 실패가 어떻게 구분되고 어떤 오류 유형으로 나타나는지 분석한다. RQ3은 최종 응답이 기대 행동인 완전 답변, 제한 답변과 거절을 얼마나 따르며 불일치가 어느 방향으로 나타나는지 살펴본다. 답변 가능성의 3단계 구분과 기대 행동과 실제 행동의 대조에는 선행 연구가 존재하므로[8,9], 본 연구는 그 구분 자체의 신규성을 주장하지 않는다.

본 연구의 기여는 다음과 같다. 첫째, 고정된 단일 대학 공식 문서 코퍼스와 운영 RAG 경로를 50문항 감사 질문 집합으로 평가한 사례를 제시한다. 둘째, 기존 RAG 평가 개념을 페이지와 청크의 계보[10], 실제 주입 근거와 표시 출처[5], 시간 및 대상자 배포 적합성, 사전 정의한 보수적 응답 정책 준수로 연결하여 적용한다. 시간 축은 시점 민감 질의를 다룬 선행 연구와 맞닿아 있으며[11,12], 대상자 축은 확인한 학술 문헌에서 선례를 찾지 못하여 본부 안내를 옮겨 실은 학과 페이지, 회차 공지와 집단별 안내라는 도메인 요구에서 도출하였다. 셋째, 이 동결 실행에서 검색 지표별 최고 구성이 서로 달랐고, 내용, 근거, 배포와 정책의 불일치가 서로 다른 문항 집합으로 나타났음을 기술한다. 넷째, qrel, 마스킹 응답, 판정 및 집계 산출물과 해시 계보를 제공하여 산출물 수준의 감사를 지원한다.
...
본 연구의 세 검색 구성은 새로운 검색 기법이 아니라 확립된 기법의 조합이다. BM25는 확률적 관련성 프레임워크에 기반한 표준 어휘 검색이며[13], 밀집 검색은 이중 인코더로 질의와 문단을 같은 벡터 공간에 사상한다[14]. 두 후보 목록의 결합에는 순위 역수 융합(Reciprocal Rank Fusion, RRF)을 사용하며, 본 연구의 상수 60은 원 논문의 예비 실험에서 선택된 값을 본 연구의 데이터로 재조정하지 않고 그대로 사용한 설정이다[15]. 다만 여러 분야의 데이터셋을 모은 벤치마크에서 단일 검색기가 항상 우세하지 않다는 관찰[16], RRF가 파라미터에 민감하며 결합이 단일 시스템보다 나빠질 수 있다는 분석[17], 비영어 특수 도메인에서 융합 효과가 조건에 의존한다는 보고[18]는 두 검색기의 결합을 보편적 기본값으로 전제할 수 없음을 보여 준다. 이에 따라 본 연구는 RQ1을 특정 구성의 우월성 검증이 아니라 동일 운영 조건에서의 회수와 순위 품질 관찰로 설계한다.

RAG 평가 연구는 검색과 생성을 잇는 계보를 다층으로 판정해 왔다. KILT는 페이지 수준 출처 평가의 원류이며[10], 문서 단위 관련성 라벨이 하위 생성 품질과 어긋날 수 있다는 지적[4]은 본 연구가 페이지와 청크의 판정을 분리하는 출발점과 문제의식을 같이한다. TREC RAG 트랙은 관련성, 완전성과 귀속(attribution)을 결합한 다층 평가와 상위 결과를 합쳐 판정하는 관행을 보여 준다[19]. 참조 없는 LLM 자동 평가[20]는 충실성 측정에 집중하며 문서의 시간 배포 가능성과 같은 배포 맥락은 다루지 않는다. 근거 표시 축에서는 생성문이 확인된 출처에 귀속 가능한지를 묻는 귀속 개념[5], 인용(citation) 품질의 자동 평가[21]와 상용 생성형 검색 엔진의 실측[22]이 이어져 왔다. 본 연구는 이 축들에, 주장을 지지하는 출처라도 시점이 만료되었거나 대상 집단이 다르면 배포에 부적합하다는 판정을 추가한다.

근거가 부족할 때 답변을 보류하는 문제는 도메인 이동 환경의 선택적 QA에서 다뤄졌고[23], RAG에서는 미응답 유형과 허용 응답을 평가하는 UAEval4RAG[7]와 문맥 충분성과 실제 행동을 교차하는 분석[24]이 제시되었다. 본 연구의 신규성 주장과 가장 가까운 경계는 MTRAG 계열이다. MTRAG는 다회전(multi-turn) 대화에서 질문을 answerable, partially answerable과 unanswerable로 구분하고 응답이 미응답(IDK)인지 판정하며[8], MTRAGEval은 여기에 부분 문항의 허용 판정과 불명확 질문에 대한 명확화 요구 판정을 더한 공동 평가 과제다[9]. 따라서 답변 가능성의 3단계 구분이나 기대와 실제 행동의 대조 자체는 본 연구의 신규 기여가 아니다. 다만 이들의 주 채점은 완전 답변 가능 문항과 부분 답변 가능 문항을 모두 답변한 경우로 함께 처리하므로, 완전 답변, 제한 답변과 거절의 정확한 3단계 기대·실제 일치를 독립 채점하고 여기에 시간과 대상자 적합성을 결합한 분해 구성이 동일하게 수행된 문헌은 확인하지 못하였다.

시간 맥락에 따라 답이 달라지는 질의는 SituatedQA가 정식화하였고[11], 빠르게 변하는 지식의 사실성 평가[12]와 낡은 정보가 RAG 정확도를 해친다는 실증[6]이 시간 축 평가의 계보를 이룬다. 반면 정부 정책 문서처럼 사용자의 조건에 따라 답의 적용 여부가 달라지는 질문은 조건부 답변 독해 과제로 제시되었으나[25], 문서의 대상자 적합성을 관련성 판정의 분리 축으로 둔 학술 문헌은 본 연구가 확인한 범위에서 찾지 못하였다. 대학 도메인 응용으로는 국내 학습관리시스템 질의응답 챗봇[2]과 해외 대학 학생 지원 에이전트[3] 등 사례가 늘고 있으나, 확인한 범위에서 이들의 평가는 검색 성공률, 사용성이나 안전성 등 단일 축에 머무르며 본 연구와 같은 분해 감사는 확인하지 못하였다.
...
평가 대상은 대학 구성원의 학사 문의에 공식 문서를 근거로 답하는 질의응답 시스템이다. 이 시스템은 단일 대학의 공식 웹 페이지와 문서를 수집하고 HTML과 PDF를 파싱하여 텍스트와 표 청크로 분할한다. 검색 입력에는 청크 본문과 함께 문서 제목과 메뉴 경로를 사용한다. 문서와 청크는 PostgreSQL에 저장하며, 활성 문서의 비어 있지 않은 청크를 의미 검색용 벡터 저장소(ChromaDB)와 BM25 색인에 등록한다.

BM25 검색은 한국어 경량 토큰화와 제한된 목록의 도메인 동의어 확장을 사용한다. Semantic 검색은 Voyage 임베딩으로 질문 벡터를 생성하여 벡터 저장소에서 유사 청크를 조회한다. Hybrid 검색은 두 후보 목록을 상수 60의 RRF로 결합한다[15]. RRF 점수는 순위 결정에 사용하고, 의미 점수와 BM25 점수의 가중 결합으로 만든 관련도는 근거 선택 단계의 절대 관련도 판정에 사용한다. 상세 질문에서는 제목형 청크 뒤의 표 청크를 보강하고 중복 결과를 제거한다.

검색 결과는 문서 단위로 묶어 최대 네 개의 근거 후보로 제한한다. 후보는 관련도가 0.35 이상이거나 질문과 직접 학사 키워드가 하나 이상 겹칠 때 통과하며, 통과 후보가 없으면 시스템은 근거 부족 응답을 반환한다. 후보가 있으면 운영 프롬프트가 답변 가능 상태(answerability), 요약, 절차, 주의 사항과 사용 출처 번호를 구조화된 JSON으로 생성하도록 요청하고, 백엔드는 JSON을 검증하고 복구한 뒤 실제 사용된 번호에 해당하는 출처만 최종 응답에 표시한다. 질문 입력에서 구조화 응답에 이르는 이 경로는 Fig. 1과 같다.
...
평가 질문은 학사 행정 영역에서 목적 표집한 50문항이다. 이 중 여섯 문항(Q021~Q026)은 2026년 7월 재학생 설문 응답에서 유도하고 나머지 44문항은 주로 연구자가 학사 행정 범위와 알려진 실패 유형이 포함되도록 작성하였다. Q026은 의도적인 거절 시험 후보로 포함하였다. 질문 유형은 사실, 절차, 일정, 연락처와 정책을 포함한다. 최종 평가 정답표 기준 기대 행동의 분포는 완전 답변 28문항, 제한 답변 3문항과 거절 19문항이다. Q011과 Q035는 같은 질문이므로 50문항 결과를 주 결과로 유지하고, Q035를 제외한 49문항 결과를 중복 민감도 분석으로 병기한다. 질문 집합은 무작위 표본이 아니며 같은 집합의 초기 실패가 시스템 개선에도 사용되었으므로, 모든 결과는 이 고정 사례에 한정하여 해석한다.

설문에서 유도한 여섯 문항은 재학생 대상 온라인 설문을 학생 단체 채팅방에 공지하여 자발적으로 수집한 자유 서술 응답에서 유도하였다. 설문은 학사 행정과 관련하여 궁금한 점을 문장 또는 키워드로 적도록 요청하였고, 보상 지급을 위한 선택적 이름 항목 외에는 식별 정보를 수집하지 않았다. 해당 이름 정보는 문항 구성과 평가 산출물에 사용하지 않았으며, 응답 원문이 아닌 질문 주제만 평가 문항으로 옮겼다.
...
판정 후보는 세 검색 구성의 동결 상위 다섯 건 합집합에 이전 판정과 예비 평가에서 관련성이 확인된 항목을 더한 pool로 구성하였다. 세 실행의 상위 다섯 건에 등장한 페이지와 청크는 채점 전에 모두 판정하였다. 이 pool은 관찰된 검색 결과를 공정하게 비교하기 위한 것이며 코퍼스 전체를 망라하는 판정이 아니다. 수집 범위 밖의 목표 출처는 별도 목록에 기록하였다.
...
서로 다른 축이 잡아낸 대표 사례는 Table 4와 같다. 표시 출처만 실패한 사례(Q002), 근거에 충실하지만 지난 회차 정보여서 배포 적합성만 실패한 사례(Q040), 대학원용 정보를 학부 질문에 적용하여 여러 축이 함께 실패한 사례(Q016)와 품질 축을 통과하고도 조건을 특정하지 않고 안내하여 부분 정확에 그친 사례(Q025)가 각각 관찰되었다.
...
같은 질문의 세 반복 실행에서 파이프라인이 반환한 구조화 응답 행동은 46/50(92.0%)에서 세 번 모두 같았고, 달라진 문항은 Q008, Q029, Q039와 Q041이었다. 이 수치는 시스템이 반환한 답변 가능 상태(answerability) 필드 범주의 반복 일치도로, 사람이 응답 텍스트를 보고 판정한 실제 행동과는 다른 변수다. 따라서 이를 사람 판정 행동의 반복 검증이나 답변 내용의 안정성으로 해석하지 않는다.
...
둘째 한계는 판정 주체와 범위다. 최종 평가 정답표는 세 검색 구성의 상위 결과와 이전에 관련성이 확인된 항목에 기반한 pooled 판정으로 코퍼스 전체를 망라하지 않으며, LLM 초벌과 단일 저자의 고위험 중심 감사는 독립적인 사람 전량 이중 판정이 아니다. 초기 라벨 비공개 감사, 단계별 마스킹과 사전 동결 기준의 통제를 두었지만, 같은 저자가 최종 평가 정답표 확정과 최종 응답 판정을 함께 수행한 역할 중첩이 남고, 감사 표본으로 전체 라벨 오류율이나 평가자 간 일치도를 추정하지 않는다. 판정은 실제 화면이 아닌 통제 평가 화면에서 이루어졌고, 세 실행 중 첫 응답만 품질을 판정하여 내용 안정성은 측정하지 않았다.
...
[3] J. Trienes, A. Derzhanskaia, R. Schwarzkopf, M. Mühling, J. Schlötterer, and C. Seifert, "Marcel: A Lightweight and Open-Source Conversational Agent for University Student Support," Proceedings of the 2025 Conference on Empirical Methods in Natural Language Processing: System Demonstrations, pp. 181-195, 2025.
...
[9] S. Rosenthal, V. Shah, Y. Katsis, and M. Danilevsky, "SemEval-2026 Task 8: MTRAGEval: Evaluating Multi-Turn RAG Conversations," Proceedings of the 20th International Workshop on Semantic Evaluation (SemEval-2026), pp. 3673-3690, 2026.
[10] F. Petroni, A. Piktus, A. Fan, P. Lewis, M. Yazdani, N. De Cao, et al., "KILT: a Benchmark for Knowledge Intensive Language Tasks," Proceedings of the 2021 Conference of the North American Chapter of the Association for Computational Linguistics: Human Language Technologies, pp. 2523-2544, 2021.
[11] M.J.Q. Zhang and E. Choi, "SituatedQA: Incorporating Extra-Linguistic Contexts into QA," Proceedings of the 2021 Conference on Empirical Methods in Natural Language Processing, pp. 7371-7387, 2021.
[12] T. Vu, M. Iyyer, X. Wang, N. Constant, J. Wei, J. Wei, et al., "FreshLLMs: Refreshing Large Language Models with Search Engine Augmentation," Findings of the Association for Computational Linguistics: ACL 2024, pp. 13697-13720, 2024.
...
[14] V. Karpukhin, B. Oguz, S. Min, P. Lewis, L. Wu, S. Edunov, et al., "Dense Passage Retrieval for Open-Domain Question Answering," Proceedings of the 2020 Conference on Empirical Methods in Natural Language Processing, pp. 6769-6781, 2020.
[15] G.V. Cormack, C.L.A. Clarke, and S. Büttcher, "Reciprocal Rank Fusion Outperforms Condorcet and Individual Rank Learning Methods," Proceedings of the 32nd International ACM SIGIR Conference on Research and Development in Information Retrieval, pp. 758-759, 2009.
[16] N. Thakur, N. Reimers, A. Rücklé, A. Srivastava, and I. Gurevych, "BEIR: A Heterogeneous Benchmark for Zero-shot Evaluation of Information Retrieval Models," Proceedings of the Neural Information Processing Systems Track on Datasets and Benchmarks 1, 2021.
[17] S. Bruch, S. Gai, and A. Ingber, "An Analysis of Fusion Functions for Hybrid Retrieval," ACM Transactions on Information Systems, Vol. 42, No. 1, Article 20, 2024.
[18] A. Louis, G. van Dijck, and G. Spanakis, "Know When to Fuse: Investigating Non-English Hybrid Retrieval in the Legal Domain," Proceedings of the 31st International Conference on Computational Linguistics, pp. 4293-4312, 2025.
[19] S. Upadhyay, N. Thakur, R. Pradeep, N. Craswell, D. Campos, and J. Lin, "Overview of the TREC 2025 Retrieval Augmented Generation (RAG) Track," Proceedings of the Thirty-Fourth Text REtrieval Conference (TREC 2025), NIST, 2026.
[20] S. Es, J. James, L. Espinosa-Anke, and S. Schockaert, "RAGAs: Automated Evaluation of Retrieval Augmented Generation," Proceedings of the 18th Conference of the European Chapter of the Association for Computational Linguistics: System Demonstrations, pp. 150-158, 2024.
[21] T. Gao, H. Yen, J. Yu, and D. Chen, "Enabling Large Language Models to Generate Text with Citations," Proceedings of the 2023 Conference on Empirical Methods in Natural Language Processing, pp. 6465-6488, 2023.
[22] N.F. Liu, T. Zhang, and P. Liang, "Evaluating Verifiability in Generative Search Engines," Findings of the Association for Computational Linguistics: EMNLP 2023, pp. 7001-7025, 2023.
[23] A. Kamath, R. Jia, and P. Liang, "Selective Question Answering under Domain Shift," Proceedings of the 58th Annual Meeting of the Association for Computational Linguistics, pp. 5684-5696, 2020.
[24] H. Joren, J. Zhang, C.-Y. Ferng, D.-C. Juan, A. Taly, and C. Rashtchian, "Sufficient Context: A New Lens on Retrieval Augmented Generation Systems," Proceedings of the 13th International Conference on Learning Representations, 2025.
