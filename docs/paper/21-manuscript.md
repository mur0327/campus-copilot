...
**상태: 전체 초안 완성. 저자 정보·제목·투고분야 확정 대기.**
**작성일 2026-07-17, 최종 갱신 2026-07-19.**
...
`[[ ]]` 표기는 미기입 자리표시자다.
본문 인용 키는 검수 종료 후 REFERENCE의 등장 순 번호로 일괄 치환하며, REFERENCE의 괄호 키도 그때 제거한다. `[[pp.]]`는 미확정 쪽수이고, 기입된 쪽수와 저자진은 최종 감사에서 전수 재확인한다.
...
이름: [[국문 저자명, 공저자·지도교수 포함 여부 확정 필요]]
소속: [[국문 소속]]
...
투고분야: 인공지능 / 머신러닝 / 딥러닝 (가안, 지도교수 검토에서 확정)

Corresponding Author: [[영문 성명]]
...
University academic information is scattered across official pages that differ in applicable time and target audience, and retrieving a relevant document does not guarantee a deployable answer. This paper reports an offline case study that audits an operational retrieval-augmented generation (RAG) pipeline for university academic guidance on a frozen corpus with 50 purposively sampled questions. We build a three-level evaluation qrel that separately records page support, chunk-level evidence, temporal validity, audience fit, and expected response behavior (full answer, qualified answer, or abstention), combining LLM first-pass judgments with label-blind human auditing followed by post-reveal adjudication. Using the same qrel, we re-score BM25-only, semantic-only, and hybrid retrieval under shared operational post-processing, and judge final responses on content accuracy, claim support, displayed sources, deployment fit, and behavior compliance. In this frozen run, no single retrieval configuration dominated all metrics. Strict composite success was 31/50 (62.0%). Relative to a conservative response policy predefined for this audit, over-answering occurred in 13 of 22 restricted-expectation questions while over-refusal was 0 of 31, and content, grounding, and deployment failures formed distinct question sets with limited three-way overlap. All results are restricted to this single-institution fixed case, and the published artifacts support artifact-level auditing of the reported numbers.
...
이는 검색 회수의 성공과 최종 응답의 적합성이 서로 다른 문제이기 때문이다. 문서 단위의 관련성 라벨이 하위 생성 품질을 보장하지 않는다는 지적이 제기되어 왔으며[eRAG], 정답 페이지가 상위에 회수되더라도 실제 주입된 청크가 질문의 핵심 주장을 지지하지 않을 수 있다. 또한 생성된 문장이 근거에 지지되더라도 표시 출처가 부정확할 수 있고[AIS], 내용이 사실이더라도 지난 회차의 안내이거나 다른 집단을 위한 문서라면 현재 답변의 근거로 사용하기 어렵다[HoH]. 나아가 근거의 충분 여부와는 별개로, 완전 답변, 제한 답변 및 거절 가운데 적절한 행동을 선택하는지도 독립된 평가 대상이다[UAEval4RAG][MTRAG]. 이러한 층위들을 하나의 점수로 합치면 실패의 위치를 특정할 수 없다.

이에 본 연구는 단일 대학의 공식 웹 문서 코퍼스와 운영 RAG 경로를 동결하고, 목적 표집한 고정 50문항에서 이러한 층위들을 분리하여 관찰하는 오프라인 사례 연구를 수행한다. 질문, 페이지 및 청크 세 수준의 평가 정답표(qrel)에 페이지 지지, 청크 근거, 시간 배포 가능성, 대상자 적합성과 질문별 기대 행동을 분리하여 기록하고, LLM 전량 초벌 판정과 초기 라벨 비공개 사람 감사, 전체 문항 검토와 불일치 조정을 거쳐 최종 평가 정답표를 확정한다. 같은 최종 평가 정답표로 BM25 단독, Semantic 단독과 Hybrid의 세 검색 구성을 재채점하고, 운영 Hybrid 경로의 최종 응답은 통제 평가 화면에서 재현한 답변 텍스트와 표시 출처로 판정하며, 같은 질문을 세 번 실행하여 구조화 행동의 반복성을 진단한다. 모든 수치와 실패 빈도는 단일 기관, 고정 코퍼스와 고정된 평가 시점의 이 사례에 귀속한다.

본 연구의 연구문제는 다음과 같다. RQ1은 공통 후처리를 공유하는 세 검색 구성이 페이지 및 근거 회수와 순위 품질에서 어떤 차이를 보이는지 검토한다. RQ2는 최종 응답에서 내용 정확성, 근거 축과 배포 적합성 실패가 어떻게 구분되고 어떤 오류 유형으로 나타나는지 분석한다. RQ3은 최종 응답이 기대 행동인 완전 답변, 제한 답변 및 거절을 얼마나 따르며 불일치가 어느 방향으로 나타나는지 살펴본다. 답변 가능성의 3단계 구분과 기대 행동과 실제 행동의 대조에는 선행 연구가 존재하므로[MTRAG][MTRAGEval], 본 연구는 그 구분 자체의 최초성을 주장하지 않는다.

본 연구의 기여는 다음과 같다. 첫째, 고정된 단일 대학 공식 문서 코퍼스와 운영 RAG 경로를 50문항 감사 문항셋으로 평가한 사례를 제시한다. 둘째, 기존 RAG 평가 개념을 페이지와 청크의 계보[KILT], 실제 주입 근거와 표시 출처[AIS], 시간 및 대상자 배포 적합성, 사전 정의한 보수적 응답 정책 준수로 연결하여 적용한다. 시간 축은 시점 민감 질의를 다룬 선행 연구에 접속하며[SituatedQA][FreshQA], 대상자 축은 확인한 학술 문헌에서 선례를 찾지 못하여 학과 미러, 회차 공지와 집단별 안내라는 도메인 요구에서 도출하였다. 셋째, 이 동결 실행에서 검색 지표별 최고 구성이 서로 달랐고, 내용, 근거, 배포와 정책의 불일치가 서로 다른 문항 집합으로 나타났음을 기술한다. 넷째, qrel, 마스킹 응답, 판정 및 집계 산출물과 해시 계보를 제공하여 산출물 수준의 감사를 지원한다.

본 논문의 구성은 다음과 같다. 2장에서는 관련 연구를 검토하고, 3장과 4장에서는 대상 시스템과 평가 방법을 각각 기술한다. 5장에서는 검색 구성 비교, 최종 응답 실패 분석과 응답 정책 준수 분석의 결과를 보고한다. 마지막으로 6장에서는 함의와 연구의 한계를 논의하고, 7장에서 결론을 맺는다.

## 2. 관련 연구

### 2.1 검색 기법

본 연구의 세 검색 구성은 확립된 기법의 운영 조합이다. BM25는 확률적 관련성 프레임워크에 기반한 표준 어휘 검색이며[BM25], 밀집 검색은 이중 인코더로 질의와 문단을 같은 벡터 공간에 사상한다[DPR]. 두 후보 목록의 결합에는 순위 역수 융합(RRF)을 사용하며, 본 연구의 상수 60은 원 논문의 파일럿에서 선택된 값을 재튜닝 없이 가져온 설정이다[RRF]. 다만 이질적 도메인에서 단일 검색기가 항상 우세하지 않다는 관찰[BEIR], RRF가 파라미터에 민감하며 결합이 단일 시스템보다 나빠질 수 있다는 분석[Bruch], 비영어 특수 도메인에서 융합 효과가 조건에 의존한다는 보고[Louis]는 하이브리드 결합을 보편적 기본값으로 전제할 수 없음을 보여 준다. 이에 따라 본 연구는 RQ1을 특정 구성의 우월성 검증이 아니라 동일 운영 조건에서의 회수와 순위 품질 관찰로 설계한다.

### 2.2 RAG 평가와 근거

RAG 평가 연구는 검색과 생성을 잇는 계보를 다층으로 판정해 왔다. KILT는 페이지 수준 출처 평가의 원류이며[KILT], 문서 단위 관련성 라벨이 하위 생성 품질과 어긋날 수 있다는 지적[eRAG]은 본 연구가 페이지와 청크의 판정을 분리하는 출발점과 같은 문제의식이다. TREC RAG 트랙은 관련성, 완전성과 attribution을 결합한 다층 평가와 pooled 판정 관행을 보여 준다[TRECRAG]. 참조 없는 LLM 자동 평가[RAGAS]는 충실성 측정에 집중하며 문서의 시간 배포 가능성과 같은 배포 맥락은 다루지 않는다. 근거 표시 축에서는 생성문이 확인된 출처에 귀속 가능한지를 묻는 attribution 개념[AIS], citation 품질의 자동 평가[ALCE]와 상용 생성형 검색 엔진의 실측[LiuVer]이 이어져 왔다. 본 연구는 이 축들 위에, 지지되는 출처라도 시점이 만료되었거나 대상 집단이 다르면 배포에 부적합하다는 판정 층을 더한다.

### 2.3 미응답·거절과 답변 행동

근거가 부족할 때 답변을 보류하는 문제는 도메인 이동 아래의 선택적 QA[Kamath]에서 다뤄졌고, RAG에서는 미응답 유형과 허용 응답을 평가하는 UAEval4RAG[UAEval4RAG]와 문맥 충분성과 실제 행동을 교차하는 분석[SuffCtx]이 제시되었다. 본 연구와 가장 가까운 경계선은 MTRAG 계열이다. MTRAG는 다회전 대화에서 질문을 answerable, partially answerable과 unanswerable로 구분하고 응답의 IDK 여부를 판정하며[MTRAG], MTRAGEval은 여기에 부분 문항의 허용 판정과 불명확 질문에 대한 명확화 요구 판정을 더한 공동 평가 과제다[MTRAGEval]. 따라서 답변 가능성의 3단계 구분이나 기대와 실제 행동의 대조 자체는 본 연구의 신규 기여가 아니다. 다만 이들의 주 채점은 완전과 부분 문항을 같은 답변 측으로 묶으므로, 완전 답변, 제한 답변과 거절의 정확한 3단계 기대·실제 일치를 독립 채점하고 여기에 시간과 대상자 적합성을 결합한 분해 구성이 동일하게 수행된 문헌은 확인하지 못하였다.

### 2.4 배포 맥락과 대학 도메인 응용

시간 맥락에 따라 답이 달라지는 질의는 SituatedQA가 정식화하였고[SituatedQA], 빠르게 변하는 지식의 사실성 평가[FreshQA]와 낡은 정보가 RAG 정확도를 해친다는 실증[HoH]이 시간 축 평가의 계보를 이룬다. 반면 문서의 대상 집단 적합성을 관련성 판정의 분리 축으로 둔 학술 문헌은 본 연구가 확인한 범위에서 찾지 못하였다. 대학 도메인 응용으로는 국내 학습관리시스템 질의응답 챗봇[Lee2024]과 해외 대학 학생 지원 에이전트[Marcel] 등 사례가 늘고 있으나, 확인한 범위에서 이들의 평가는 검색 성공률, 사용성이나 안전성 등 단일 축에 머무르며 본 연구와 같은 분해 감사는 확인하지 못하였다.

### 2.5 관련성 판정 방법론

고정된 시스템들의 상위 결과를 합쳐 판정하는 pooled qrel은 확립된 평가 관행이지만 pool 밖 문서에 대한 완전성을 보장하지 않는다[TREC2021]. LLM을 관련성 판정에 활용하는 연구는 비용 효율과 시스템 순위 수준의 상관을 보고하는 한편[UMBRELA], LLM 판정이 사람 판정을 대체하기 어렵고[ClarkeDietz] 서로 다른 LLM 판정기가 사람보다 자기들끼리 더 강하게 합의한다는 공유 편향 경고[Frobe]도 제기되었다. LLM을 보조 수단으로 두고 사람이 고위험 사례를 감사해야 한다는 관점[Faggioli]에 따라, 본 연구는 LLM 전량 초벌 뒤 초기 라벨 비공개 사람 감사와 조정을 결합하되 이 절차를 방법론적 기여가 아니라 제한된 pool에서의 보수적 사례 연구 절차로 사용한다.

## 3. 대상 시스템

평가 대상이 되는 시스템 경로는 Fig. 1과 같다.
...
BM25 검색은 한국어 경량 토큰화와 제한된 도메인 동의어 확장을 사용한다. Semantic 검색은 Voyage 임베딩으로 질문 벡터를 생성하여 벡터 저장소에서 유사 청크를 조회한다. Hybrid 검색은 두 후보 목록을 상수 60의 Reciprocal Rank Fusion(RRF)으로 결합한다[RRF]. RRF 점수는 순위 결정에 사용하고, 의미 점수와 BM25 점수의 가중 결합으로 만든 관련도는 근거 선택 단계의 절대 관련도 판정에 사용한다. 상세 질문에서는 제목형 청크 뒤의 표 청크를 보강하고 중복 결과를 제거한다.

### 3.3 근거 선택과 응답 생성

검색 결과는 문서 단위로 묶어 최대 네 개의 근거 후보로 제한한다. 후보는 관련도가 0.35 이상이거나 질문과 직접 학사 키워드가 하나 이상 겹칠 때 통과하며, 통과 후보가 없으면 시스템은 근거 부족 응답을 반환한다. 후보가 있으면 운영 프롬프트가 답변 가능 상태, 요약, 절차, 주의 사항과 사용 출처 번호를 구조화된 JSON으로 생성하도록 요청하고, 백엔드는 JSON을 검증하고 복구한 뒤 실제 사용된 번호에 해당하는 출처만 최종 응답에 표시한다.

### 3.4 실행 조건과 코퍼스 동결

검색 비교(RQ1)는 같은 코퍼스에서 BM25 단독, Semantic 단독과 Hybrid 경로를 연속 실행한 동결 결과를 사용한다. 단일 검색 구성은 해당 검색기만 활성화하고 관련도 결합 가중치를 1.0으로 두며, 상세 컨텍스트 확장과 중복 제거 등 공통 운영 후처리는 유지한다. 큐레이션 고정 결과(Best Bets)만 끄고 각 구성의 최종 반환 결과 중 상위 다섯 건을 채점한다. 최종 응답 실행은 운영 구성 그대로 Best Bets를 켠 상태에서 수행하였으며, 사용 모델과 후보 수를 포함한 세부 실행 조건은 Table 1의 각주에 정리한다. 코퍼스는 실행 전후의 지문 비교로 동결을 확인하였고, 동결 시점 기준 활성 문서 4,087개와 색인 청크 20,788개를 포함한다.
...
판정 후보는 세 검색 구성의 동결 top-5 합집합에 이전 판정과 예비 평가에서 확인된 양성 seed를 더한 pool로 구성하였다. 세 실행의 top-5에 등장한 페이지와 청크는 채점 전에 모두 판정하였다. 이 pool은 관찰된 검색 결과를 공정하게 비교하기 위한 것이며 코퍼스 전체를 망라하는 판정이 아니다. 수집 범위 밖의 목표 출처는 별도 목록에 기록하였다.
...
최종 발행본은 페이지 판정 424행, 청크 근거 626행과 질문 판정 50행으로 구성되며, 발행 직후 12개 불변 조건 검사를 통과하고 세 검색 실행의 top-5에 미판정 후보가 없음을 확인하였다.
...
정상 응답이 기대 행동과 일치하고, 기권이면 명확한 근거 부족 고지가 있으며, 내용이 완전 정확하거나 순수 기권으로 해당 없음이고, 주장이 실제 주입 근거에 완전 지지되거나 해당 없음이며, 표시 출처가 완전하거나 해당 없음이고, 응답 주장 시간 유효성, 대상자 및 개인정보 배포 적합성을 모두 통과할 때만 엄격한 종합 성공으로 계수한다. 부분 정확과 부분 지지는 엄격한 종합 성공에 포함하지 않고 별도로 보고한다. 이 정의는 사전 동결한 다축 기준의 결합 준수이며 일반 정확도나 실제 사용자 성공률이 아니다.
...
이 동결 검색 실행에서는 지표별 최고 구성이 서로 달랐다(Table 3). Hybrid는 완전 지지 페이지와 완전 본문 근거를 하나 이상 회수한 비율이 가장 높았으나 그 차이는 Semantic 대비 각각 한 문항 규모였고, Semantic은 부분 지지를 포함한 회수율과 두 nDCG에서 가장 높았다. BM25 단독은 모든 지표에서 가장 낮았다. Hybrid가 완전 근거 회수를 추가한 유일한 문항(Q005)의 후보는 BM25가 제공하였으나 해당 페이지는 시점이 만료되어 기대 행동이 거절이며, Semantic만 부분 지지 페이지를 회수한 문항(Q024)에서는 BM25의 비관련 결과가 결합되면서 해당 페이지가 상위 다섯 건 밖으로 밀렸다. 이 실행에서는 완전 근거의 최소 한 건 회수와 전체 관련 근거의 순위 품질 사이의 trade-off가 관찰되었으며, 이를 융합의 일반적 기제나 인과 효과로 해석하지 않는다.
...
서로 다른 축이 포착한 대표 사례는 Table 4와 같다. 표시 출처만 실패한 사례(Q002), 근거에 충실하지만 지난 회차 정보여서 배포 적합성만 실패한 사례(Q040), 대학원용 정보를 학부 질문에 적용하여 여러 축이 함께 실패한 사례(Q016)와 품질 축을 통과하고도 조건 특정 없이 안내하여 부분 정확에 그친 사례(Q025)가 각각 관찰되었다.
...
최종 응답에서는 내용 정확성, 결합 근거와 배포 적합성의 실패 집합이 제한적으로만 겹쳤고, 행동 오류는 과잉거절이 아니라 과잉답변 방향으로 나타났다. 과잉답변 13/22문항 중 3문항은 품질 축을 모두 통과한 정책 전용 불일치였다는 점에서, 이 축은 정확성이나 근거성과 구분되는 배포 정책 준수의 독립 관찰이다. 다만 이 빈도는 목적 표집 감사 문항셋의 결과이며 실제 서비스 질의 분포의 과잉답변률을 추정하지 않는다. 분해형 평가는 서로 다른 실패 원인을 각각 기록하는 방식으로, 본 연구는 이를 한 사례에 구성하고 적용하였다.
...
둘째 한계는 판정 주체와 범위다. 최종 평가 정답표는 세 검색 구성의 상위 결과와 양성 seed에 기반한 pooled 판정으로 코퍼스 전체를 망라하지 않으며, LLM 초벌과 단일 저자의 위험 기반 감사는 독립적인 사람 전량 이중 판정이 아니다. 초기 라벨 비공개 감사, 단계별 마스킹과 사전 동결 기준의 통제를 두었지만, 같은 저자가 최종 평가 정답표 확정과 최종 응답 판정을 함께 수행한 역할 중첩이 남고, 감사 표본으로 전체 라벨 오류율이나 평가자 간 일치도를 추정하지 않는다. 판정은 실제 화면이 아닌 통제 평가 화면에서 이루어졌고, 세 실행 중 첫 응답만 품질을 판정하여 내용 안정성은 측정하지 않았다.

셋째 한계는 두 평가의 검색 범위 차이다. 최종 응답의 운영 검색(Best Bets 포함 상위 여섯 건)과 최종 평가 정답표의 pool(Best Bets 제외 상위 다섯 건 중심)이 완전히 같지 않아, 실제 주입 근거 가운데 페이지 2쌍과 청크 2개(Q010, Q049)가 미판정으로 남았다. RQ1의 검색 지표는 상위 다섯 건의 완전 판정을 강제하였으므로 이 차이의 영향을 받지 않으며, 잔여 영향은 해당 두 문항의 최종 응답 판정에서 참조 근거의 완전성에 한정된다.

평가는 2026년 7월의 최종 평가 정답표와 실행 시점에 고정되었으며 이후의 문서 갱신에 따라 같은 질문의 기대 행동과 결과가 달라질 수 있다. 개인정보 축은 판정된 위험 응답이 0건이어서 위험 탐지 성능 자체를 평가하지 못하였고, 이 결과가 시스템이 원칙적으로 개인정보를 생성하지 않는다는 보장은 아니다. 본 연구의 공개 산출물은 최종 평가 정답표, 세 검색 구성의 동결 출력 공개본, 마스킹 응답과 판정·집계 결과의 해시 계보를 통한 산출물 수준의 감사를 지원하며, 완전한 재실행 재현을 제공하는 것은 아니다.

## 7. 결론

본 연구는 단일 대학의 공식 문서 코퍼스와 운영 RAG 경로를 동결하고, 목적 표집한 고정 50문항에서 검색 회수와 최종 응답의 내용, 근거, 배포 및 행동 적합성을 분리하여 관찰하였다. 세 수준으로 분해한 최종 평가 정답표와 감사 절차를 구성하였고, 이 동결 실행에서 검색 지표별 최고 구성이 서로 달랐으며, 최종 응답의 내용 정확성, 결합 근거와 배포 적합성 실패가 서로 다른 문항 집합으로 나타나고 행동 오류가 과잉답변 방향으로 관찰되었음을 보고하였다. 이러한 결과는 검색 성공률이나 단일 정확도만으로는 기관 안내 서비스의 배포 가능성을 판단하기 어렵고, 응답 정책 준수를 별도의 축으로 감사할 필요가 있음을 이 사례 안에서 보여 준다. 향후 과제로 검색 구성별 최종 응답 품질 비교, 코퍼스 확장과 다기관 검증, 독립 평가자에 의한 신뢰도 측정, 시점 갱신 기능의 인과 효과 평가와 거절 정책의 사용자 수용성 검증을 남긴다.
...
[25] (TREC2021) I. Soboroff, "Overview of TREC 2021," Proceedings of the Thirtieth Text REtrieval Conference (TREC 2021), NIST, 2021.
[26] (UMBRELA) S. Upadhyay, R. Pradeep, N. Thakur, D. Campos, N. Craswell, I. Soboroff, et al., "A Large-Scale Study of Relevance Assessments with Large Language Models Using UMBRELA," Proceedings of the 2025 International ACM SIGIR Conference on Innovative Concepts and Theories in Information Retrieval (ICTIR), pp. 358-368, 2025.
[27] (ClarkeDietz) C.L.A. Clarke and L. Dietz, "LLM-based Relevance Assessment Still Can't Replace Human Relevance Assessment," Proceedings of the 18th International Workshop on Evaluating Information Access (EVIA 2025), [[pp.]], 2025.
[28] (Frobe) M. Fröbe, A. Parry, F. Schlatt, S. MacAvaney, B. Stein, M. Potthast, et al., "Large Language Model Relevance Assessors Agree With One Another More Than With Human Assessors," Proceedings of the 48th International ACM SIGIR Conference on Research and Development in Information Retrieval, pp. 2858-2863, 2025.
[29] (Faggioli) G. Faggioli, L. Dietz, C.L.A. Clarke, G. Demartini, M. Hagen, C. Hauff, et al., "Perspectives on Large Language Models for Relevance Judgment," Proceedings of the 2023 ACM SIGIR International Conference on Theory of Information Retrieval, pp. 39-50, 2023.
