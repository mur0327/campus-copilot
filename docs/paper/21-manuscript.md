...
대학의 학사 정보는 학사 안내 페이지, 공지사항, 학과 및 부속 기관 사이트에 흩어져 있으며, 같은 주제를 다루는 문서라도 적용 시점과 대상 집단이 서로 다르다. 본부 안내를 옮겨 실은 학과 페이지, 회차마다 갱신되는 모집 공지, 신입생이나 졸업예정자와 같이 특정 집단만을 위한 안내가 하나의 코퍼스 안에 공존한다. 검색 증강 생성(Retrieval-Augmented Generation, RAG)[1]은 이러한 공식 문서를 근거로 질문에 답하는 접근으로 대학 학사 안내에도 적용되고 있으나[2,3], 관련 문서의 검색에 성공하였다고 해서 사용자에게 그대로 내보낼 수 있는 답변이 보장되는 것은 아니다.

이는 검색 회수의 성공과 최종 응답의 적합성이 서로 다른 문제이기 때문이다. 문서 단위의 관련성 라벨이 하위 생성 품질을 보장하지 않는다는 지적이 제기되어 왔으며[4], 정답 페이지가 상위에 회수되더라도 실제 주입된 청크가 질문의 핵심 주장을 지지하지 않을 수 있다. 또한 생성된 문장이 근거에 지지되더라도 표시 출처가 부정확할 수 있고[5], 내용이 사실이더라도 지난 회차의 안내이거나 다른 집단을 위한 문서라면 현재 답변의 근거로 사용하기 어렵다[6]. 나아가 근거의 충분 여부와는 별개로, 완전 답변, 제한 답변과 거절 중에서 적절한 행동을 선택하는지도 독립된 평가 대상이다[7,8]. 이러한 층위들을 하나의 점수로 합치면 실패의 위치를 특정할 수 없다.
...
본 연구의 세 검색 구성은 확립된 기법의 운영 조합이다. BM25는 확률적 관련성 프레임워크에 기반한 표준 어휘 검색이며[13], 밀집 검색은 이중 인코더로 질의와 문단을 같은 벡터 공간에 사상한다[14]. 두 후보 목록의 결합에는 순위 역수 융합(Reciprocal Rank Fusion, RRF)을 사용하며, 본 연구의 상수 60은 원 논문의 예비 실험에서 선택된 값을 재튜닝 없이 가져온 설정이다[15]. 다만 이질적 도메인에서 단일 검색기가 항상 우세하지 않다는 관찰[16], RRF가 파라미터에 민감하며 결합이 단일 시스템보다 나빠질 수 있다는 분석[17], 비영어 특수 도메인에서 융합 효과가 조건에 의존한다는 보고[18]는 하이브리드 결합을 보편적 기본값으로 전제할 수 없음을 보여 준다. 이에 따라 본 연구는 RQ1을 특정 구성의 우월성 검증이 아니라 동일 운영 조건에서의 회수와 순위 품질 관찰로 설계한다.

RAG 평가 연구는 검색과 생성을 잇는 계보를 다층으로 판정해 왔다. KILT는 페이지 수준 출처 평가의 원류이며[10], 문서 단위 관련성 라벨이 하위 생성 품질과 어긋날 수 있다는 지적[4]은 본 연구가 페이지와 청크의 판정을 분리하는 출발점과 문제의식을 같이한다. TREC RAG 트랙은 관련성, 완전성과 귀속(attribution)을 결합한 다층 평가와 pooled 판정 관행을 보여 준다[19]. 참조 없는 LLM 자동 평가[20]는 충실성 측정에 집중하며 문서의 시간 배포 가능성과 같은 배포 맥락은 다루지 않는다. 근거 표시 축에서는 생성문이 확인된 출처에 귀속 가능한지를 묻는 귀속 개념[5], 인용(citation) 품질의 자동 평가[21]와 상용 생성형 검색 엔진의 실측[22]이 이어져 왔다. 본 연구는 이 축들 위에, 지지되는 출처라도 시점이 만료되었거나 대상 집단이 다르면 배포에 부적합하다는 판정 층을 더한다.

근거가 부족할 때 답변을 보류하는 문제는 도메인 이동 환경의 선택적 QA에서 다뤄졌고[23], RAG에서는 미응답 유형과 허용 응답을 평가하는 UAEval4RAG[7]와 문맥 충분성과 실제 행동을 교차하는 분석[24]이 제시되었다. 본 연구의 신규성 주장과 가장 가까운 경계는 MTRAG 계열이다. MTRAG는 다회전 대화에서 질문을 answerable, partially answerable과 unanswerable로 구분하고 응답이 미응답(IDK)인지 판정하며[8], MTRAGEval은 여기에 부분 문항의 허용 판정과 불명확 질문에 대한 명확화 요구 판정을 더한 공동 평가 과제다[9]. 따라서 답변 가능성의 3단계 구분이나 기대와 실제 행동의 대조 자체는 본 연구의 신규 기여가 아니다. 다만 이들의 주 채점은 완전과 부분 문항을 같은 답변 측으로 묶으므로, 완전 답변, 제한 답변과 거절의 정확한 3단계 기대·실제 일치를 독립 채점하고 여기에 시간과 대상자 적합성을 결합한 분해 구성이 동일하게 수행된 문헌은 확인하지 못하였다.
