...
이에 본 연구는 단일 대학의 공식 웹 문서 코퍼스와 운영 RAG 파이프라인을 동결하고, 연구 목적에 따라 선정하여 고정한 50문항에서 이러한 층위들을 분리하여 관찰하는 오프라인 사례 연구를 수행한다. 평가 정답표(qrel)는 질문별로 어떤 페이지와 청크가 근거가 되는지를 기록한 판정 자료다. 본 연구는 이를 질문, 페이지 및 청크의 세 수준으로 나누어 페이지 지지, 청크 근거, 시간 배포 가능성, 대상자 적합성과 질문별 기대 행동을 분리하여 기록한다. 최종 평가 정답표는 LLM이 전체 후보를 초벌 판정한 뒤, 초기 라벨을 공개하지 않고 사람이 다시 판정하는 감사와 전체 문항 검토, 불일치 조정을 거쳐 확정한다. 이렇게 확정한 정답표로 BM25 단독, Semantic 단독과 Hybrid의 세 가지 검색 구성을 재채점하고, 운영 Hybrid 구성의 최종 응답은 통제 평가 화면에서 재현한 답변 텍스트와 표시 출처로 판정한다. 아울러 같은 질문을 세 번 실행하여 시스템이 반환한 구조화 응답 행동이 실행마다 일치하는지를 진단한다. 모든 수치와 실패 빈도는 단일 기관에서 코퍼스와 평가 시점을 고정한 이 사례에 한정된다. 그 범위 안에서 본 연구는 층위를 분리한 감사가 실패 지점을 짚어 내고 응답의 배포 가능성, 즉 실제 안내에 내보낼 수 있는지를 판단하는 데 필요함을 보인다.
...
본 연구는 단일 대학의 공식 문서 코퍼스와 운영 RAG 파이프라인을 동결하고, 연구 목적에 따라 선정하여 고정한 50문항에서 검색 회수와 최종 응답의 내용 정확성, 결합 근거, 배포 적합성과 응답 정책 준수를 분리하여 관찰하였다. 또한 질문, 페이지 및 청크의 세 수준으로 구성한 최종 평가 정답표와 감사 절차를 제시하였다. 이 동결 실행에서는 어느 검색 구성도 모든 지표에서 우세하지 않았다. 최종 응답에서는 내용 정확성, 결합 근거와 배포 적합성 세 축의 실패가 서로 다른 문항 집합으로 나타났고, 행동 불일치는 과잉답변 방향으로 관찰되었다. 이러한 결과는 검색 성공률이나 단일 정확도만으로는 기관 안내 서비스의 배포 가능성을 판단하기 어렵고, 응답 정책 준수를 별도의 축으로 감사할 필요가 있음을 이 사례 안에서 보여 준다.

다만 본 연구는 단일 기관의 고정 코퍼스와 연구 목적에 따라 선정한 50문항을 사용하였다. 또한 같은 질문 집합에서 발견된 초기 실패가 시스템 개선에 사용되었다. 따라서 보고한 빈도와 비율은 다른 기관, 언어, 모델이나 서비스로 일반화할 수 없는 고정 사례 감사로 해석해야 한다. 최종 평가 정답표는 pool 기반 판정이므로 코퍼스 전체를 망라하지 않는다. LLM 초벌 판정 뒤 단일 저자가 고위험 사례와 표본을 감사하였으므로, 독립된 평가자 두 사람이 전체 후보를 이중 판정하는 절차는 보완할 점으로 남는다. 전체 실험을 동일하게 재실행할 수 있음을 보장하지는 않지만, 본 연구는 최종 평가 정답표, 세 가지 검색 구성의 동결 출력과 마스킹된 응답 자료를 공개하였다. 판정·집계 결과 사이의 관계를 추적할 수 있는 해시 기록도 제공하였다. 이 공개 범위는 공개 산출물만으로 보고된 수치를 점검하는 산출물 수준 감사를 지원한다.
