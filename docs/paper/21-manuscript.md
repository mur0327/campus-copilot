...
**상태: 2026-08-06 투고 완료. 심사 결과 대기.**
**작성일 2026-07-17, 최종 갱신 2026-08-06.**
이 문서는 투고본과 같은 내용이나 Table 3의 2행과 3행만 다르다. 투고본의 두 행은 이전 판본이므로 수정본에서 교체한다(submission/next-revision.md).
...
`[[ ]]` 표기는 자리표시자다. 저자명, 소속과 연락처는 개인정보이므로 저장소에 기록하지 않고 투고 파일에만 채운다.
...
국문 제목: 대학 학사 문서 기반 RAG 시스템의 배포 적합성 다층 평가

이름: [[국문 저자명 — 지도교수 공저 확정(교신저자), 표기 순서 확인 필요]]
소속: [[국문 소속]]

영문 제목: Multi-Level Evaluation of Deployability for a RAG System Based on University Academic Documents
...
이에 본 연구는 단일 대학의 공식 웹 문서 코퍼스와 운영 RAG 파이프라인을 동결하고, 연구 목적에 따라 선정하여 고정한 50문항에서 이러한 층위들을 분리하여 관찰하는 오프라인 사례 연구를 수행한다. 평가 정답표(qrel)는 질문별로 어떤 페이지와 청크가 근거가 되는지를 기록한 판정 자료다. 본 연구는 이를 질문, 페이지 및 청크의 세 수준으로 나누어 페이지 지지, 청크 근거, 시간 배포 가능성, 대상자 적합성과 질문별 기대 행동을 분리하여 기록한다. 최종 평가 정답표는 LLM이 전체 후보를 초벌 판정한 후, 초기 라벨을 공개하지 않은 상태에서 사람이 독립적으로 재판정하고, 전체 문항 검토와 판정 불일치 조정 과정을 거쳐 확정하였다. 이렇게 확정한 정답표로 BM25 단독, Semantic 단독과 Hybrid의 세 가지 검색 구성을 재채점하고, 운영 Hybrid 구성의 최종 응답은 통제 평가 화면에서 재현한 답변 텍스트와 표시 출처로 판정한다. 아울러 같은 질문을 세 번 실행하여 시스템이 반환한 구조화 응답 행동이 실행마다 일치하는지를 진단한다. 모든 수치와 실패 빈도는 단일 기관에서 코퍼스와 평가 시점을 고정한 이 사례에 한정된다. 그 범위 안에서 본 연구는 층위를 분리한 감사가 실패 지점을 짚어 내고 응답의 배포 가능성, 즉 실제 안내에 내보낼 수 있는지를 판단하는 데 필요함을 보인다.
...
여기서 f(t,d)는 질의 단어 t가 문서 d에 나타난 빈도, IDF(t)는 역문서 빈도, |d|와 avgdl은 문서 길이와 평균 문서 길이이며, k₁과 b는 빈도 포화와 길이 정규화를 조절하는 파라미터다. 이 점수는 질의에 나온 단어가 문서에 실제로 등장할 때만 발생하므로, 내용이 관련된 문서라도 질의와 어휘가 다르면 점수를 얻지 못한다. 밀집 검색은 이중 인코더로 질의와 문단을 같은 벡터 공간에 나타내어 이러한 어휘 불일치를 보완할 수 있다[10]. 두 후보 목록의 결합에는 순위 역수 융합(Reciprocal Rank Fusion, RRF)을 사용하며, 문서 d의 결합 점수는 식 (2)와 같다[11].

$$RRF(d)=\sum_{r \in R}\frac{1}{k+rank_r(d)} \qquad (2)$$

여기서 R은 결합할 순위 목록의 집합, rank_r(d)는 목록 r에서 문서 d의 순위이며, 상수 k로는 원 논문의 예비 실험에서 선택된 60을 본 연구의 데이터로 재조정하지 않고 그대로 사용한다[11]. 식 (2)의 점수는 각 목록의 순위 역수 합이므로, 한쪽 검색기에서만 상위인 문서는 다른 쪽 목록의 기여가 없어 결합 순위가 낮아질 수 있다. 실제로 여러 분야의 데이터셋을 모은 벤치마크에서 단일 검색기가 항상 우세하지 않다는 관찰[12], RRF가 파라미터에 민감하며 결합이 단일 시스템보다 나빠질 수 있다는 분석[13], 비영어 특수 도메인에서 융합 효과가 조건에 의존한다는 보고[14]는 두 검색기의 결합이 언제나 더 나은 선택이라고 전제할 수 없음을 보여 준다. 이에 따라 본 연구는 검색 구성 비교를 특정 구성의 우월성 검증이 아니라, 동일한 운영 조건에서 회수와 순위 품질을 관찰하는 실험으로 설계한다.

RAG 평가 연구는 검색에서 생성으로 이어지는 과정을 여러 층위로 나누어 판정해 왔다. KILT는 페이지 수준 출처 평가를 정립한 초기 연구다[15]. 본 연구는 문서 단위 관련성 라벨이 이후 생성 단계의 품질과 어긋날 수 있다는 지적[4]과 문제의식을 공유하여 페이지와 청크의 판정을 분리한다. TREC RAG 트랙은 관련성, 완전성과 귀속(attribution)을 결합한 다층 평가를 제시한다[16]. 여기서 귀속은 생성된 문장을 확인된 출처로 되짚을 수 있는지를 뜻한다. 참조 없는 LLM 자동 평가[17]는 충실성 측정에 집중하며 문서의 시간 배포 가능성과 같은 배포 적합성 축은 다루지 않는다. 근거 표시 축에서는 귀속 개념을 정식화한 연구[5], 인용(citation) 품질의 자동 평가[18]와 상용 생성형 검색 엔진의 실측[19]이 이어져 왔다. 본 연구는 이러한 근거 표시 축에 배포 적합성 판정을 더하여, 주장을 지지하는 출처라도 시점이 만료되었거나 대상 집단이 다르면 배포에 부적합한 것으로 간주한다.

근거가 부족할 때 답변을 보류하는 문제는 도메인 이동 환경의 선택적 질의응답(selective QA)에서 다뤄졌고[20], RAG에서는 미응답 유형과 허용 응답을 평가하는 UAEval4RAG[7]와 문맥 충분성과 실제 행동을 교차하는 분석[21]이 제시되었다. 본 연구의 신규성 주장과 가장 가까운 선행 연구는 MTRAG 계열이다. MTRAG는 다회전(multi-turn) 대화에서 질문을 답변 가능(answerable), 부분 답변 가능(partially answerable)과 답변 불가능(unanswerable)으로 구분하고, 응답이 미응답(IDK)인지를 판정한다[8]. MTRAGEval은 여기에 부분 답변 가능 문항의 허용 판정과 불명확한 질문에 대한 명확화 요구 판정을 더한 공동 평가 과제다[22]. 따라서 답변 가능성의 3단계 구분이나 기대와 실제 행동의 대조 자체는 본 연구의 신규 기여가 아니다. 다만 MTRAG 계열의 주 채점은 완전 답변 가능 문항과 부분 답변 가능 문항을 모두 답변한 경우로 함께 처리하는 반면, 본 연구는 완전 답변, 제한 답변과 거절의 3단계 기대·실제 일치를 독립 채점한다. 여기에 시간과 대상자 적합성을 별도의 축으로 분리하여 기대 행동과 함께 평가한 문헌은 확인하지 못하였다.
...
여러 시스템의 상위 결과를 합쳐 판정 대상 집합(pool)을 만들고 그 안에서만 관련성을 판정하는 pooled qrel은 확립된 평가 관행이지만, pool 밖 문서에 대한 완전성을 보장하지 않는다[26]. LLM을 관련성 판정에 활용하는 연구는 비용 효율과 함께, 사람 판정으로 매긴 시스템 순위와 높은 상관을 보고한다[27]. 한편으로는 LLM 판정이 사람 판정을 대체하기 어렵다는 반론[28]과 서로 다른 LLM 판정기 사이의 합의가 각 판정기와 사람 판정 사이의 합의보다 높아 공통 편향을 공유할 수 있다는 경고[29]도 제기되었다. LLM을 보조 수단으로 두고 사람이 고위험 사례를 감사해야 한다는 관점[30]에 따라, 본 연구는 LLM이 후보 전량을 초벌 판정한 뒤 초기 라벨을 공개하지 않은 사람 감사를 수행하고, 이후 라벨을 공개한 상태에서 차이를 조정한다. 다만 이 절차는 방법론적 기여가 아니라, 제한된 pool 안에서 판정을 보수적으로 확정하기 위한 사례 연구 절차다.
...
BM25 검색은 형태소 분석기를 사용하지 않고 정규식으로 분리한 한국어 토큰에 식 (1)의 점수를 적용하며, 미리 정한 목록 안에서만 도메인 동의어를 확장한다. Semantic 검색은 2장의 밀집 검색에 해당하며, Voyage 임베딩 모델로 질문 벡터를 만들어 벡터 저장소에서 유사한 청크를 조회한다. Hybrid 검색은 두 후보 목록을 식 (2)의 RRF로 결합하며 상수 k는 60이다[11]. RRF 점수는 결과의 순위를 정하는 데에만 사용하고, 후보가 기준값을 넘는지 판별하는 데에는 의미 점수와 BM25 점수를 가중 결합한 관련도를 따로 사용한다. 학점이나 기간 같은 구체적인 값을 묻는 질문에서는 제목만 있고 내용이 거의 없는 청크 뒤에 오는 표 청크를 함께 보강하고, 중복된 결과는 제거한다.
...
검색 구성 비교는 같은 코퍼스에서 BM25 단독, Semantic 단독과 Hybrid를 연속적으로 실행한 동결 결과를 사용한다. 단일 검색 구성은 해당 검색기만 활성화하고 관련도 결합 가중치를 1.0으로 두며, 상세 컨텍스트 확장과 중복 제거 등 공통 운영 후처리는 유지한다. 특정 질의에 대해 미리 지정한 문서를 상위에 고정하는 큐레이션 결과(Best Bets)만 비활성화하고 각 구성의 최종 반환 결과 중 상위 다섯 건을 채점한다. 최종 응답 실행은 운영 구성 그대로 Best Bets를 활성화한 상태에서 수행하였다. 실행 시점을 포함한 세부 실행 조건은 4장의 Table 2 각주에 함께 정리한다. 코퍼스는 실행 전후의 해시값 비교로 내용이 바뀌지 않았음을 확인하였고, 이 시점 기준 활성 문서 4,087개와 색인 청크 20,788개를 포함한다. 평가는 이 동결 시점에 고정되며, 이후의 문서 갱신에 따라 같은 질문의 기대 행동과 결과가 달라질 수 있다. 평가에 사용한 시스템 구성 요소와 검색 설정은 Table 1과 같다.
...
평가 질문은 학사 행정 영역에서 연구 목적에 따라 선정한 50문항으로, 여섯 문항은 2026년 7월 재학생 설문에 나온 질문 주제를 옮겨 만들었고, 나머지 44문항은 학사 행정의 여러 영역과 이미 알려진 실패 유형이 고루 들어가도록 연구자가 작성하였다. 설문은 자발적 참여 방식의 자유 서술형으로 진행하였으며, 응답 원문이 아닌 질문 주제만 평가 문항으로 옮기고 식별 정보는 문항 구성과 평가 산출물에 사용하지 않았다. 최종 평가 정답표 기준 기대 행동의 분포는 완전 답변 28문항, 제한 답변 3문항과 거절 19문항이다. Q011과 Q035는 같은 질문이다. 이 중복은 실행과 정답표를 50문항 기준으로 동결한 뒤에 확인되었고, 동결한 산출물에서 문항을 사후에 빼지 않기 위하여 50문항 결과를 주 결과로 유지한다. 대신 Q035를 제외한 49문항 결과를 함께 보고하여 이 중복이 수치를 바꾸는지 확인한다. 질문 집합은 무작위 표본이 아니며, 이 집합에서 처음 발견한 실패가 시스템 개선에도 사용되었다.

평가 정답표(qrel)는 질문, 페이지 및 청크의 세 수준으로 구성한다. 질문 수준에는 기대 행동, 그리고 기대 행동이 완전 답변이 아닌 경우의 주된 사유와 부차적 사유를 기록한다. 페이지 수준에는 그 페이지가 답을 얼마나 지지하는지(지지 정도), 현재 안내에 사용할 수 있는 시점인지(시간 배포 가능성), 질문자가 그 문서의 대상에 해당하는지(대상자 적합성)를 기록한다. 청크 수준에는 근거 강도와 근거 유형을 기록하되, 완전 근거와 부분 근거에만 양성 라벨을 주고 나머지는 근거로 인정하지 않는다. 근거 유형은 본문 청크(text chunk)와 그 페이지에 도달하는 것 자체가 답이 되는 경우(page navigation)로 나눈다. 각 수준의 라벨 값과 최종 응답의 판정 축은 Table 2와 같다.
...
| Question | Primary and secondary reason (non-full-answer) | missing required claim / acquisition failure / stale / absent from pool / audience mismatch / personalized / policy exclusion |
...
판정 후보 pool은 세 가지 검색 구성이 각각 반환한 동결 상위 다섯 건을 모두 합쳐 만들고, 여기에 본평가 전에 별도로 수행한 판정과 예비 평가에서 관련성이 확인된 항목을 더하였다. 상위 다섯 건에 등장한 페이지와 청크는 채점 전에 모두 판정하였다. 이 pool은 관찰된 검색 결과를 공정하게 비교하기 위한 것이며 코퍼스 전체를 포괄하는 판정이 아니다. 코퍼스 수집 범위 밖에 있어 색인되지 않은 목표 출처는 별도 목록에 기록하였다. 한편, 최종 응답의 운영 검색은 Best Bets를 포함한 상위 여섯 건을 사용하므로, 운영 검색이 실제로 주입한 근거 중 pool 밖 항목인 질문-페이지 묶음 2개와 청크 2개(Q010, Q049)는 미판정으로 남는다. 검색 지표는 상위 다섯 건을 모두 판정하도록 강제하므로 이 미판정 항목의 영향을 받지 않는다. 남는 영향은 Q010과 Q049의 최종 응답을 판정할 때 참조 근거가 완전하지 않을 수 있다는 점에 한정된다.

판정은 세 단계로 나누어 진행하였고, 그 절차는 Table 3과 같다. 먼저 두 LLM(Claude Fable 5와 GPT 5.6 Sol)이 사전에 고정한 판정 지침에 따라 pool의 모든 후보에 초벌 판정을 부여하였고, 이 초벌 판정 원본은 이후 수정하지 않고 동결하였다. 다음으로 사람 검토자가 초기 라벨, 선정 사유, 검색 방식과 순위 정보를 확인할 수 없는 상태에서 87개 묶음을 재판정하였다. 한 묶음은 하나의 질문과 하나의 페이지, 그리고 그 페이지에 속한 청크 근거를 함께 판정하는 단위이며, 대상은 사전 정의한 고위험 사례 67개와 층화 표본 20개다. 이 표본은 양성 판정과 음성 판정으로 나누어 고정 시드로 추출하였다. 마지막으로 라벨을 공개한 상태에서 같은 저자가 사람 재판정과 초벌 판정의 차이를 조정하고 50문항의 기대 행동과 사유를 전량 확인하였다.
...
### 4.2 최종 응답 평가 설계 및 판정 절차

질문별 기대 행동은 완전 답변, 제한 답변과 거절의 세 가지이며, 기대 행동이 완전 답변이 아닌 문항에는 그 사유를 함께 기록하였다. 확정된 50문항 가운데 제한 답변이나 거절이 기대되는 문항은 22문항이고, 여기에서 관찰된 사유는 다섯 가지다. 관련 내용은 있으나 질문의 핵심 사실이나 절차를 완성하지 못한 필수 주장 부족이 8문항, 목표 출처는 있으나 코퍼스에 해당 페이지나 문서가 없는 수집 실패가 5문항, 지난 회차 정보여서 실행일에 사용할 수 없는 시점 만료가 4문항이다. 질문의 초점을 다루는 근거를 찾지 못한 판정 후보 pool 내 근거 부재가 4문항, 다른 집단을 위한 정보여서 적용할 수 없는 대상자 불일치가 1문항이다. 이 밖에 개인화와 정책 제외를 사유 코드로 정의하였으나 이 50문항에서는 관찰되지 않았다.
...
판정 축은 실제 응답 행동과 여섯 가지 품질 축이다. 품질 축 가운데 내용 정확성은 답변이 사실과 맞는지를 판정한다. 근거 축인 내용 지지(claim support)는 답변이 실제 주입된 근거로 뒷받침되는지를, 표시 출처(source display)는 화면에 표시한 출처가 실제로 사용한 근거와 일치하는지를 판정한다. 배포 적합성 축은 응답 주장 시간 유효성, 대상자 적합성과 개인정보의 셋으로 나뉜다. 각각 응답에 포함된 시간 민감 주장의 유효성, 질문자 집단에 대한 적용 가능성, 개인정보 노출 여부를 판정한다. 내용 지지와 표시 출처는 어느 하나만 실패해도 근거 제시가 성립하지 않으므로, 두 축을 묶어 결합 근거 축으로 함께 집계한다. 이때 응답의 시간 축은 문서의 시간 축과 구분된다. 최종 평가 정답표의 시간 배포 가능성이 문서를 현재 안내에 사용할 수 있는지를 판정하는 반면, 응답 주장 시간 유효성은 응답이 실제로 말한 시간 민감 주장이 유효한지를 판정한다.

판정 기준, 프롬프트, 최종 평가 정답표와 실행 원본 해시는 응답 판정 전에 동결하였다. 판정은 단계별 마스킹으로 진행하였다. 첫 단계에서는 질문, 답변 텍스트와 표시 출처만으로 실제 행동을 판정한 뒤 그 결과를 잠갔다. 다음 단계에서는 이 실행에서 실제로 주입된 근거와 최종 평가 정답표의 참조 묶음을 함께 공개하여 품질 축을 판정하였으며, 참조 묶음의 라벨은 공개하지 않았다. 모든 축을 잠근 뒤에만 기대 행동과 반복 결과를 공개하였다. 이러한 단계별 순서는 기대 행동을 먼저 알고 판정하는 데에서 오는 편향을 막기 위한 것이다. 첫 응답 50건은 단일 저자가 이 절차로 판정하였다. 50문항을 같은 고정 순서로 세 번 실행하여 응답 150건을 얻었으며, 첫 응답만 본평가에 사용하고, 두 번째와 세 번째 응답은 시스템이 반환한 답변 가능 상태가 실행마다 일치하는지를 진단하는 데에만 사용하였다.

엄격한 종합 성공은 다음 네 조건을 모두 만족한 응답만 계수한다. 실제 행동이 기대 행동과 일치하고, 거절이면 근거가 부족하다는 고지가 분명해야 한다. 내용은 완전 정확하거나, 사실 안내 없이 근거 부족만 밝힌 순수 거절이어서 해당 없음이어야 한다. 내용 지지와 표시 출처는 각각 완전하거나 해당 없음이어야 한다. 배포 적합성 축인 응답 주장 시간 유효성, 대상자 적합성과 개인정보는 모두 통과해야 한다. 부분 정확과 부분 지지는 엄격한 종합 성공에 포함하지 않고 별도로 보고한다. 이 정의는 사전에 동결한 여러 축의 기준을 동시에 만족하였는지를 뜻하며, 일반 정확도나 실제 사용자 성공률이 아니다.

분리 평가의 전체 구성은 Fig. 3과 같다. Fig. 3의 (a)부터 (d)는 각각 페이지·청크 회수, 내용 정확성과 근거, 배포 적합성, 행동 정책 준수의 판정을 나타낸다.
...
본 장에서는 4장의 절차로 얻은 결과를 제시한다. 5.1은 검색 구성 비교(RQ1), 5.2는 최종 응답 실패 분석(RQ2), 5.3은 응답 정책 준수 분석(RQ3)을 다루며, 각 결과의 함의를 함께 고찰한다.

검색 평가는 관련 페이지의 회수 여부를 확인하는 PageHit, 페이지의 순위 품질을 측정하는 nDCG, 본문 청크 근거의 회수 여부를 확인하는 EvidenceHit의 세 가지 지표군으로 나뉘며, 지표별 분모는 질문 특성에 따라 다르다. PageHit full@5는 완전 지지 페이지가, PageHit any@5는 완전 또는 부분 지지 페이지가 상위 다섯 건에 하나 이상 포함된 질문의 비율이다. nDCG support@5는 페이지의 지지 정도를 기준으로 순위 품질을 측정한다. 이 지표는 완전 지지에 2, 부분 지지에 1, 비지지에 0의 이득값(gain)을 부여한다. 같은 URL에서는 첫 번째 청크에만 이득을 인정한다. nDCG deployable@5는 지지 페이지 가운데 시간 배포 가능성이 유효하고 대상자가 일치하는 페이지에만 지지 이득을 부여한다. EvidenceHit full@5는 완전 본문 청크 근거의 회수를, EvidenceHit any@5는 완전 또는 부분 근거의 회수를 측정한다. 지지 페이지가 있는 질문은 45문항, 현재 배포 가능한 페이지가 있는 질문은 39문항, 본문 청크 근거로 평가하는 질문은 38문항이다. 그 페이지에 도달하는 것 자체가 답이 되는 질문은 근거 지표의 분모에서 제외한다.

이 동결 검색 실행에서는 가장 높은 값을 보인 검색 구성이 지표마다 달랐다. 세 가지 구성의 지표별 결과는 Table 4와 같으며, Fig. 4는 이를 막대그래프로 나타내고 있다. Hybrid는 완전 지지 페이지와 완전 본문 근거를 하나 이상 회수한 비율이 가장 높았으나, Semantic과의 차이는 두 지표 모두 한 문항이었다. Semantic은 부분 지지를 포함한 회수율과 두 nDCG에서 가장 높았다. BM25 단독은 모든 지표에서 가장 낮았다. Hybrid가 Semantic보다 완전 근거를 추가로 회수한 유일한 문항은 Q005였다. 이 근거는 BM25가 제공한 후보에서 나왔지만, 해당 페이지는 시점이 만료되어 기대 행동이 거절이었다. 반대로 Q024에서는 Semantic만 부분 지지 페이지를 상위 다섯 건에 포함하였고, Hybrid에서는 BM25의 비관련 결과가 결합되면서 이 페이지가 상위 다섯 건 밖으로 밀렸다. 이 실행에서는 완전 근거를 적어도 하나 회수하는 성능과 전체 관련 근거의 순위 품질 사이에서 상충 관계(trade-off)가 관찰되었으며, 이를 융합 방식의 일반적 특성이나 인과 효과로 해석하지 않는다.
...
중복 문항인 Q035를 제외한 49문항 분석에서도 모든 지표의 구성 순위가 유지되었고 최대 절대 변화는 0.0192였다. 또한 최종 평가 정답표 대신 LLM 초벌 정답표를 사용하여 같은 동결 검색 결과를 다시 채점함으로써 사람 감사 전후의 민감도를 확인하였다. 이 분석에서도 여섯 가지 지표의 구성 순위가 유지되었으며 최대 절대 변화는 0.0526이었다. 이 분석은 사람 감사에서 최종 반영한 판정 변경의 영향만을 보여 주며, 최종 평가 정답표의 타당성을 입증하지는 않는다. 다만 두 민감도 분석에서 구성 순위가 모두 유지되었다는 점은, 이 분석 범위에서 중복 문항 처리와 사람 감사에서 최종 반영한 정답표 조정이 구성 순위를 바꾸지 않았음을 나타낸다. 한편 RQ1의 비교는 Best Bets를 비활성화한 상위 다섯 건 기준이고 최종 응답 실행은 Best Bets를 활성화한 운영 조건이다. 따라서 두 결과는 직접 연결하지 않고, 서로 다른 조건에서 얻은 별도 관찰로 해석한다.

앞서 관찰한 상충 관계를 어떻게 해석할지는 생성 단계의 설계에 따라 달라진다. 생성 단계가 여러 근거 후보를 함께 받는 현재 설계에서는 완전 근거를 적어도 하나 확보하는 쪽이 상대적으로 중요할 수 있으나, 본 연구는 검색 구성별 최종 응답 품질을 직접 비교하지 않았다. 다만 이 실행에서 어떤 지표를 기준으로 삼느냐에 따라 구성 간 우열이 달라졌다는 점은, 검색 성능을 하나의 지표로 요약할 때 다른 지표에서 나타난 성능 저하를 함께 보고해야 함을 나타낸다.

### 5.2 최종 응답 실패 분석 (RQ2)

첫 응답 50건은 대부분의 품질 축에서 완전 충족으로 판정되었고, 실패는 축마다 다른 크기로 나타났다. 내용 정확성은 완전 정확 30건, 부분 정확 9건, 부정확 3건과 해당 없음 8건이었다. 실제 주입 근거의 내용 지지는 완전 지지 40건, 부분 지지 2건과 해당 없음 8건이었고, 표시 출처는 완전 38건, 부정확 4건과 해당 없음 8건으로 나타났다. 응답 주장 시간 유효성은 유효 6건, 만료 3건, 불명 1건과 해당 없음 40건이었다. 대상자 적합성은 일치 40건, 불일치 1건, 불명 1건과 해당 없음 8건이었고, 개인정보는 50건 모두 안전이었다. 여섯 축의 분포는 Fig. 5와 같으며, 각 막대는 첫 응답 50건이 완전 충족, 부분 충족, 미충족 또는 불명, 해당 없음으로 나뉘는 비율을 나타내고 있다.

![Fig. 5](figures/fig5-response-quality.svg)

Fig. 5. Distribution of the six response quality axes over the 50 first responses.

순수 거절 8건은 구체적인 사실 정보나 출처를 제시하지 않았으므로 내용 정확성, 내용 지지와 표시 출처의 세 축을 해당 없음으로 판정하였다. 응답 주장 시간 유효성의 해당 없음 40건은 응답에 시간 민감 주장이 포함되지 않은 경우다. 개인정보 위험으로 판정된 응답은 0건이었다. 다만 위험 사례가 없었으므로 이 축은 위험 탐지 성능이 아니라 이번 실행에서 위험 응답이 관찰되지 않았다는 사실만을 기록한다.

엄격 기준을 적용했을 때 실패 문항은 내용 정확성 12문항, 결합 근거 축 6문항과 배포 적합성 축 6문항이었다. 결합 근거 축 실패 6문항 중 Q016과 Q031은 내용 지지가 부분적이었다. 나머지 Q002, Q020, Q033과 Q041은 내용 지지는 완전했지만 표시 출처가 부정확하였다. 이는 주입 근거가 답을 뒷받침하는 것과 그 근거가 올바른 출처로 표시되는 것이 별개의 문제임을 보여 준다. 내용 정확성과 결합 근거 축, 내용 정확성과 배포 적합성 축의 교집합은 각각 4문항이었고, 세 축을 모두 실패한 문항은 Q016과 Q031뿐이었다. 세 실패 집합의 겹침은 크지 않았으며, 같은 50문항에서 각 평가 축이 포착한 실패는 서로 달랐다. 세 축의 실패를 합치면 16문항이다. 이 가운데 표시 출처가 부정확한 2문항과 시점이 만료되었거나 불명인 2문항은 내용이 완전 정확하므로, 내용 정확성 축만으로는 실패로 드러나지 않는다. 축별 대표 사례는 Table 5와 같다.
...
기대 행동과 실제 행동의 행렬 및 기대 행동별 엄격한 종합 성공은 Table 6과 같다. 행동이 일치한 문항은 36/50이었다. 여기에 거절 시 근거 부족 고지와 내용, 근거, 출처, 배포 적합성 조건까지 모두 충족한 엄격한 종합 성공은 31/50(62.0%)이었다. 기대 행동별로는 완전 답변 기대 문항이 22/28(78.6%)로 가장 높았고, 제한 답변과 거절 기대 문항은 각각 1/3(33.3%)과 8/19(42.1%)였다. Q035를 제외한 민감도 분석에서 엄격한 종합 성공은 30/49(61.2%)였다.
...
| Qualified answer (3) | 2 | 1 | 0 | 1/3 (33.3%) |
| Abstention (19) | 6 | 5 | 8 | 8/19 (42.1%) |
| Total (50) | 35 | 7 | 8 | 31/50 (62.0%) |

행동 불일치는 주로 한 방향으로 나타났다. 기대된 행동보다 더 적극적으로 답변한 과잉답변은 제한 답변 또는 거절이 기대된 22문항 중 13문항(59.1%)에서 관찰되었다. 반면 기대된 행동과 달리 거절한 과잉거절은 완전 답변 또는 제한 답변이 기대된 31문항 가운데 0건이었다. 완전 답변이 기대된 28문항 중 불필요한 제한 답변은 1건이었다. 과잉답변 13문항은 두 유형으로 나뉜다. 정책 전용 불일치(policy-only mismatch)는 여섯 가지 품질 축을 모두 통과했지만 정책 행동만 일치하지 않은 3문항(Q003, Q005, Q030)이었다. 실질 실패(substantive failure)는 하나 이상의 품질 축 실패를 동반한 10문항이었다. 과잉답변은 사전 정의한 보수적 응답 정책과의 불일치이며 그 자체로 오답이나 위험 답변을 뜻하지 않는다. 두 유형의 문항 수는 공개된 판정 자료를 다시 집계하여 얻은 값이며, 관찰된 행동 불일치의 방향은 현재 질문 집합과 구성에 한정된다.

파이프라인이 반환한 구조화 응답 행동의 반복 일치도는 시스템이 반환한 답변 가능 상태(answerability) 필드의 범주가 세 번의 실행에서 일치했는지를 나타낸다. 이 값은 사람이 응답 텍스트로 판정한 실제 행동의 재현성이나 답변 내용 자체의 안정성을 뜻하지 않는다. 같은 질문을 세 번 실행했을 때 범주가 모두 일치한 문항은 46/50(92.0%)이었고, 달라진 문항은 Q008, Q029, Q039와 Q041이었다.

응답 정책 준수는 내용 정확성이나 근거 축과 구분되는 축으로 나타났다. 최종 응답에서 내용 정확성, 결합 근거와 배포 적합성 세 축의 실패 집합은 제한적으로만 겹쳤고, 행동 불일치는 과잉거절이 아니라 과잉답변 방향으로 나타났다. 과잉답변 13문항이라는 빈도는 실제 서비스 질의 분포의 과잉답변율을 추정하지 않는다. 다만 이 가운데 정책 전용 불일치 3문항은 여섯 가지 품질 축을 모두 통과하고도 정책 기준을 벗어난 경우이므로, 품질 축의 판정만으로는 드러나지 않는다. 이는 응답 행동을 별도의 축으로 기록해야 함을 나타낸다.

## 6. 결  론

본 연구는 특정 대학의 공식 문서 코퍼스와 운영 RAG 파이프라인을 동결하고, 연구 목적에 따라 선정하여 고정한 50문항에서 검색 회수와 최종 응답의 내용 정확성, 결합 근거, 배포 적합성과 응답 정책 준수를 분리하여 평가하였다. 또한 질문, 페이지 및 청크의 세 수준으로 구성한 최종 평가 정답표와 판정 절차를 제시하였다. 이 동결 실행에서는 어느 검색 구성도 모든 지표에서 우세하지 않았다. 최종 응답에서는 내용 정확성, 결합 근거와 배포 적합성 세 축의 실패가 서로 다른 문항 집합으로 나타났고, 행동 불일치는 과잉답변 방향으로 관찰되었다. 이러한 결과는 검색 성공률이나 단일 정확도만으로는 기관 안내 서비스의 배포 가능성을 판단하기 어렵고, 응답 정책 준수를 별도의 축으로 평가할 필요가 있음을 이 사례 안에서 보여 준다.

다만 본 연구는 단일 기관의 고정 코퍼스와 연구 목적에 따라 선정한 50문항을 사용하였다. 또한 같은 질문 집합에서 발견된 초기 실패가 시스템 개선에 사용되었다. 따라서 제시한 빈도와 비율은 다른 기관, 언어, 모델이나 서비스로 일반화할 수 없는 고정 사례 분석 결과로 해석해야 한다. 최종 평가 정답표는 pool 기반 판정이므로 코퍼스 전체를 포괄하지 않는다. LLM 초벌 판정 뒤 단일 저자가 고위험 사례와 표본을 검토하였으므로, 독립된 평가자 두 사람이 전체 후보를 이중 판정하는 절차는 보완할 점으로 남는다. 전체 실험을 동일하게 재실행할 수 있음을 보장하지는 않지만, 본 연구는 최종 평가 정답표, 세 가지 검색 구성의 동결 출력과 마스킹된 응답 자료를 공개하였다. 판정·집계 결과 사이의 관계를 추적할 수 있는 해시 기록도 제공하였다. 이 공개 범위는 공개 산출물만으로 제시된 수치를 재확인할 수 있는 산출물 수준의 검증을 지원한다.
