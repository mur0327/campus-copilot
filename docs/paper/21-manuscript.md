...
판정은 세 단계로 나누어 진행하였고, 그 절차는 Table 3과 같다. 먼저 두 LLM(Claude Fable 5와 GPT 5.6 Sol)이 사전에 고정한 판정 지침(코드북)에 따라 pool의 모든 후보에 초벌 판정을 부여하고, 그 결과를 이후 수정 없이 동결하였다. 다음으로 사람 검토자가 초기 라벨, 선정 사유, 검색 방식과 순위 정보를 확인할 수 없는 상태에서 87개 묶음을 재판정하였다. 한 묶음은 하나의 질문과 하나의 페이지, 그리고 그 페이지에 속한 청크 근거를 함께 판정하는 단위이며, 대상은 사전 정의한 고위험 사례 67개와 고정 시드로 추출한 층화 표본 20개다. 마지막으로 라벨을 공개한 상태에서 같은 저자가 두 판정의 차이를 조정하고 50문항의 기대 행동과 사유를 전량 확인하였다.
...
중복 문항인 Q035를 제외한 49문항 분석에서도 모든 지표의 구성 순위가 유지되었고 최대 절대 변화는 0.0192였다. 또한 최종 평가 정답표 대신 LLM 초벌 정답표를 사용하여 같은 동결 검색 결과를 다시 채점함으로써 사람 감사 전후의 민감도를 확인하였다. 이 분석에서도 여섯 가지 지표의 구성 순위가 유지되었으며 최대 절대 변화는 0.0526이었다. 이 분석은 사람 감사에서 최종 반영한 판정 변경의 영향만을 보여 주며, 최종 평가 정답표의 타당성을 입증하지는 않는다. 다만 두 민감도 분석에서 구성 순위가 모두 유지되었다는 점은 관찰된 구성 간 차이가 중복 문항 처리나 정답표 조정 범위에 좌우되지 않았음을 나타낸다. 한편 RQ1의 비교는 Best Bets를 비활성화한 상위 다섯 건 기준이고 최종 응답 실행은 Best Bets를 활성화한 운영 조건이다. 따라서 두 결과는 직접 연결하지 않고, 서로 다른 조건에서 얻은 별도 관찰로 해석한다.
...
순수 거절 8건은 구체적인 사실 정보나 출처를 제시하지 않았으므로 내용 정확성, 내용 지지와 표시 출처 축을 해당 없음으로 판정하였다. 응답 주장 시간 유효성의 해당 없음 40건은 응답에 시간 민감 주장이 포함되지 않은 경우다. 개인정보 위험으로 판정된 응답은 0건이었다. 다만 위험 사례가 없었으므로 이 축은 위험 탐지 성능이 아니라 이번 실행에서 위험 응답이 관찰되지 않았다는 사실만을 기록한다.
...
기대 행동과 실제 행동의 행렬 및 기대 행동별 엄격한 종합 성공은 Table 6과 같다. 행동이 일치한 문항은 36/50이었고, 내용, 근거, 출처와 배포 적합성까지 모두 충족한 엄격한 종합 성공은 31/50(62.0%)이었다. 기대 행동별로는 완전 답변 기대 문항이 22/28(78.6%)로 가장 높았고, 제한 답변과 거절 기대 문항은 각각 1/3(33.3%)과 8/19(42.1%)였다. Q035를 제외한 민감도 분석에서 엄격한 종합 성공은 30/49(61.2%)였다.
...
응답 정책 준수는 내용 정확성이나 근거 축과 구분되는 축으로 나타났다. 최종 응답에서 내용 정확성, 결합 근거와 배포 적합성의 실패 집합은 제한적으로만 겹쳤고, 행동 오류는 과잉거절이 아니라 과잉답변 방향으로 나타났다. 특히 과잉답변 13문항 가운데 정책 전용 불일치 3문항은 내용과 근거, 출처와 배포 축을 모두 통과하고도 정책 기준을 벗어난 경우였다. 과잉답변 13문항이라는 빈도가 실제 서비스 질의 분포의 과잉답변률을 추정하지는 않지만, 이러한 문항은 품질 축의 판정만으로는 드러나지 않으므로 응답 행동을 별도의 축으로 기록해야 함을 나타낸다.

## 6. 결  론

본 연구는 단일 대학의 공식 문서 코퍼스와 운영 RAG 파이프라인을 동결하고, 연구 목적에 따라 선정하여 고정한 50문항에서 검색 회수와 최종 응답의 내용, 근거, 배포 및 행동 적합성을 분리하여 관찰하였다. 또한 질문, 페이지 및 청크의 세 수준으로 구성한 최종 평가 정답표와 감사 절차를 제시하였다. 이 동결 실행에서는 어느 검색 구성도 모든 지표에서 우세하지 않았다. 최종 응답에서는 내용 정확성, 결합 근거 축과 배포 적합성 축의 실패가 서로 다른 문항 집합으로 나타났고, 행동 오류는 과잉답변 방향으로 관찰되었다. 이러한 결과는 검색 성공률이나 단일 정확도만으로는 기관 안내 서비스의 배포 가능성을 판단하기 어렵고, 응답 정책 준수를 별도의 축으로 감사할 필요가 있음을 이 사례 안에서 보여 준다.

다만 본 연구는 단일 기관의 고정 코퍼스와 연구 목적에 따라 선정한 50문항을 사용하였다. 또한 같은 질문 집합에서 발견된 초기 실패가 시스템 개선에 사용되었다. 따라서 보고한 빈도와 비율은 다른 기관, 언어, 모델이나 서비스로 일반화할 수 없는 고정 사례 감사로 해석해야 한다. 최종 평가 정답표는 pool 기반 판정이므로 코퍼스 전체를 망라하지 않는다. LLM 초벌 판정 뒤 단일 저자가 고위험 사례와 표본을 감사했으므로, 독립된 평가자 두 사람이 전체 후보를 이중 판정하지는 않았다는 보완할 점도 남아 있다. 전체 실험을 동일하게 재실행할 수 있음을 보장하지는 않지만, 본 연구는 최종 평가 정답표, 세 가지 검색 구성의 동결 출력과 마스킹된 응답 자료를 공개하였다. 판정·집계 결과 사이의 관계를 추적할 수 있는 해시 기록도 제공하였다. 이 공개 범위는 공개 산출물만으로 보고된 수치를 점검하는 산출물 수준 감사를 지원한다.
