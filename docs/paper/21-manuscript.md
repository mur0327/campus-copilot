...
여러 시스템의 상위 결과를 합쳐 판정 대상 집합(pool)을 만들고 그 안에서만 관련성을 판정하는 pooled qrel은 확립된 평가 관행이지만, pool 밖 문서에 대한 완전성을 보장하지 않는다[26]. LLM을 관련성 판정에 활용하는 연구는 비용 효율과 함께, 사람 판정으로 매긴 시스템 순위와 높은 상관을 보고한다[27]. 한편으로는 LLM 판정이 사람 판정을 대체하기 어렵다는 반론[28]과 서로 다른 LLM 판정기 사이의 합의가 각 판정기와 사람 판정 사이의 합의보다 높아 공통 편향을 공유할 수 있다는 경고[29]도 제기되었다. LLM을 보조 수단으로 두고 사람이 고위험 사례를 감사해야 한다는 관점[30]에 따라, 본 연구는 LLM이 후보 전량을 초벌 판정한 뒤 초기 라벨을 공개하지 않은 사람 감사와 조정을 결합한다. 다만 이 절차는 방법론적 기여가 아니라, 제한된 pool 안에서 판정을 보수적으로 확정하기 위한 사례 연구 절차다.
...
엄격한 종합 성공은 다음 네 조건을 모두 만족한 응답만 계수한다. 실제 행동이 기대 행동과 일치하고, 거절이면 근거가 부족하다는 고지가 분명해야 한다. 내용은 완전 정확하거나, 사실 안내 없이 근거 부족만 밝힌 순수 거절이어서 해당 없음이어야 한다. 내용 지지와 표시 출처는 각각 완전하거나 해당 없음이어야 한다. 응답 주장 시간 유효성, 대상자 적합성과 개인정보 배포 적합성은 모두 통과해야 한다. 부분 정확과 부분 지지는 엄격한 종합 성공에 포함하지 않고 별도로 보고한다. 이 정의는 사전에 동결한 여러 축의 기준을 동시에 만족하였는지를 뜻하며, 일반 정확도나 실제 사용자 성공률이 아니다.
...
기대 행동과 실제 행동의 행렬 및 기대 행동별 엄격한 종합 성공은 Table 6과 같다. 행동이 일치한 문항은 36/50이었고, 그중 거절 시 근거 부족 고지와 내용, 근거, 출처, 배포 적합성 조건까지 모두 충족한 엄격한 종합 성공은 31/50(62.0%)이었다. 기대 행동별로는 완전 답변 기대 문항이 22/28(78.6%)로 가장 높았고, 제한 답변과 거절 기대 문항은 각각 1/3(33.3%)과 8/19(42.1%)였다. Q035를 제외한 민감도 분석에서 엄격한 종합 성공은 30/49(61.2%)였다.
...
응답 정책 준수는 내용 정확성이나 근거 축과 구분되는 축으로 나타났다. 최종 응답에서 내용 정확성, 결합 근거와 배포 적합성 세 축의 실패 집합은 제한적으로만 겹쳤고, 행동 불일치는 과잉거절이 아니라 과잉답변 방향으로 나타났다. 과잉답변 13문항이라는 빈도는 실제 서비스 질의 분포의 과잉답변율을 추정하지 않는다. 다만 이 가운데 정책 전용 불일치 3문항은 내용, 근거, 출처와 배포의 네 축을 모두 통과하고도 정책 기준을 벗어난 경우이므로, 품질 축의 판정만으로는 드러나지 않는다. 이는 응답 행동을 별도의 축으로 기록해야 함을 나타낸다.
