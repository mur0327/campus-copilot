...
평가 정답표(qrel)는 질문, 페이지 및 청크의 세 수준으로 구성한다. 질문 수준에는 기대 행동, 그리고 완전 답변이 어려운 경우의 주된 사유와 부차적 사유를 기록한다. 페이지 수준에는 그 페이지가 답을 얼마나 지지하는지(지지 정도), 현재 안내에 사용할 수 있는 시점인지(시간 배포 가능성), 질문자가 그 문서의 대상에 해당하는지(대상자 적합성)를 기록한다. 청크 수준에는 근거 강도와 근거 유형을 기록하되, 완전 근거와 부분 근거에만 양성 라벨을 주고 나머지는 근거로 인정하지 않는다. 근거 유형은 본문 청크(text chunk)와 그 페이지에 도달하는 것 자체가 답이 되는 경우(page navigation)로 나눈다. 각 수준의 라벨 값과 최종 응답의 판정 축은 Table 2와 같다.
...
판정은 세 단계로 나누어 진행하였고, 그 절차는 Table 3과 같다. 먼저 두 LLM(Claude Fable 5와 GPT 5.6 Sol)이 사전에 고정한 판정 지침(코드북)에 따라 pool의 모든 후보에 초벌 판정을 부여하였고, 이 초벌 판정 원본은 이후 수정하지 않고 동결하였다. 다음으로 사람 검토자가 초기 라벨, 선정 사유, 검색 방식과 순위 정보를 확인할 수 없는 상태에서 87개 묶음을 재판정하였다. 한 묶음은 하나의 질문과 하나의 페이지, 그리고 그 페이지에 속한 청크 근거를 함께 판정하는 단위이며, 대상은 사전 정의한 고위험 사례 67개와 고정 시드로 추출한 층화 표본 20개다. 마지막으로 라벨을 공개한 상태에서 같은 저자가 사람 재판정과 초벌 판정의 차이를 조정하고 50문항의 기대 행동과 사유를 전량 확인하였다.
...
질문별 기대 행동은 완전 답변, 제한 답변과 거절의 세 가지이며, 완전 답변이 어려운 경우에는 그 주된 사유를 함께 기록하였다. 확정된 50문항 가운데 제한 답변이나 거절이 기대되는 문항은 22문항이고, 여기에서 관찰된 사유는 다섯 가지다. 관련 내용은 있으나 질문의 핵심 사실이나 절차를 완성하지 못한 필수 주장 부족이 8문항, 목표 출처는 있으나 코퍼스에 해당 페이지나 문서가 없는 수집 실패가 5문항, 지난 회차 정보여서 실행일에 사용할 수 없는 시점 만료가 4문항이다. 질문의 초점을 다루는 근거를 찾지 못한 판정 후보 pool 내 근거 부재가 4문항, 다른 집단을 위한 정보여서 적용할 수 없는 대상자 불일치가 1문항이다. 이 밖에 개인화와 정책 제외를 사유 코드로 정의하였으나 이 50문항에서는 관찰되지 않았다.
...
엄격 기준을 적용했을 때 실패 문항은 내용 정확성 12문항, 결합 근거 축 6문항과 배포 적합성 축 6문항이었다. 결합 근거 축 실패 6문항 중 Q016과 Q031은 내용 지지가 부분적이었다. 나머지 Q002, Q020, Q033과 Q041은 내용 지지는 완전했지만 표시 출처가 부정확하였다. 이는 주입 근거가 답을 뒷받침하는 것과 그 근거가 올바른 출처로 표시되는 것이 별개의 문제임을 보여 준다. 내용 정확성과 결합 근거 축, 내용 정확성과 배포 적합성 축의 교집합은 각각 4문항이었고, 세 축을 모두 실패한 문항은 Q016과 Q031뿐이었다. 세 실패 집합의 겹침은 크지 않았으며, 같은 50문항에서 각 평가 축이 포착한 실패는 서로 달랐다. 축별 대표 사례는 Table 5와 같다.
